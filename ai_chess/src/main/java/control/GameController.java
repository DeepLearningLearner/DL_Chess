...
public class GameController {
    ...
    public void moveChess(String key, int[] position, Board board) {
        ...
        // 同时需要更新自己和对方.
        board.getPlayer(key.charAt(0)).updatePiece(key, position);
        board.getEnemy(key.charAt(0)).killPiece(position);
    ...
    public void responseMoveChess(Board board, GameView view, char color) {
        /**
         * Implements artificial intelligence.
         * */
        Player player = board.getPlayer(color);
        BaseStepNode result = player.play();

        view.movePieceFromAI(result.piece, result.to);

        // 先更新玩家棋子，后更新棋局.
        board.getPlayer(color).updatePiece(result.piece, result.to);
        board.getEnemy(color).killPiece(result.to);

        board.updatePiece(result.piece, result.to);

        board.addTrace(result.piece);

        board.changePlayer();
    }

    public void responseMoveChess(Board board, char color) {
        /**
         * Implements artificial intelligence.
         * */
        Player player = board.getPlayer(color);
