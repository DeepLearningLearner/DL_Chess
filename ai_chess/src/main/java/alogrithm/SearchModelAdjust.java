 ...
 * Alpha beta search.
 */
public class SearchModelAdjust implements Search{
    ...
    private EvalModelAdjust evalModel;

    public SearchModelAdjust(){
    	this.evalModel = new EvalModelAdjust();
    }
    ...
    private int alphaBeta(int depth, int alpha, int beta, boolean isMax, final char player) {
        ...
        if (depth == 0 || Rules.hasWin(this.thinkingBoard) != 'x')
        	return this.evalModel.eval(this.thinkingBoard, player);
