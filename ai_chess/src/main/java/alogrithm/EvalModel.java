    ...
    public EvalModel(){
    	for(int i = 0; i < Board.BOARD_HEIGHT; i++){
    		for(int j = 0; j < Board.BOARD_WIDTH; j++){
    			this.pPosition[i][j] = this.pPosition[i][j] + random.nextInt(9) - 4;
    			this.mPosition[i][j] = this.mPosition[i][j] + random.nextInt(9) - 4;
    			this.jPosition[i][j] = this.jPosition[i][j] + random.nextInt(11) - 5;
    			this.zPosition[i][j] = this.zPosition[i][j] + random.nextInt(3) - 1;
    		}
    	}
    }
